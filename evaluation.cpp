...
double evaluate(Board& board) {
    ...
    for(int i=0; i<6; i++) {
        for(int j=0; j<64; j++) {
            whitePos += board.bitboards[i] & (1ULL << j) ? mg_pieceSquareTable[i][j] : 0;
            whiteMaterial += board.bitboards[i] & (1ULL << j) ? pieceValues[i] : 0;
        }
    }
    ...
    for(int i=6; i<12; i++) {
        for(int j=0; j<64; j++) {
            blackPos += board.bitboards[i] & (1ULL << j) ? mg_pieceSquareTable[i][j] : 0;
            blackMaterial += board.bitboards[i] & (1ULL << j) ? pieceValues[i] : 0;
        }
    }
    ...
    const double positionScalar = 1169.0; // this is the highest amount of points possible given a full board
